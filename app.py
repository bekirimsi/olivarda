"""
Zeytinyağı Alım ve Cari Takip Uygulaması
=========================================
Streamlit tabanlı, SQLite veritabanı kullanan web uygulaması.
"""

import streamlit as st
import sqlite3
import os
import hashlib
import uuid
from datetime import datetime, date
from pathlib import Path

# ============================================================
# YAPILANDIRMA
# ============================================================
DB_PATH = "olivearda.db"
UPLOAD_DIR = "uploads"
DEFAULT_USER = "admin"
DEFAULT_PASS = "1234"

st.set_page_config(
    page_title="Olivarda | Erengül Zeytinyağı",
    page_icon="🫒",
    layout="wide",
    initial_sidebar_state="collapsed",
)

# Kurumsal kimlik renkleri
OLIVE_GREEN = "#6B8E23"
OLIVE_DARK = "#556B2F"

# KPI kart stilleri + kurumsal kimlik CSS
st.markdown(f"""
<style>
/* --- Kurumsal Logo / Başlık --- */
.brand-header {{
    text-align: center;
    padding: 8px 0 4px 0;
}}
.brand-name {{
    font-size: 1.65rem;
    font-weight: 800;
    color: {OLIVE_GREEN};
    letter-spacing: 1.5px;
    line-height: 1.2;
}}
.brand-subtitle {{
    font-size: 0.72rem;
    color: #6c757d;
    font-weight: 500;
    letter-spacing: 0.3px;
    margin-top: 2px;
}}
.brand-divider {{
    width: 50px;
    height: 3px;
    background: {OLIVE_GREEN};
    border-radius: 2px;
    margin: 8px auto 0 auto;
}}
/* --- Footer --- */
.sidebar-footer {{
    text-align: center;
    font-size: 0.65rem;
    color: #adb5bd;
    padding: 12px 0 4px 0;
    border-top: 1px solid #e9ecef;
    margin-top: 16px;
}}
/* --- KPI Kartları --- */
.kpi-grid {{
    display: flex;
    flex-wrap: wrap;
    gap: 12px;
    margin: 12px 0 20px 0;
}}
.kpi-card {{
    flex: 1 1 180px;
    max-width: 100%;
    background: #f8f9fa;
    border: 1px solid #e9ecef;
    border-top: 3px solid {OLIVE_GREEN};
    border-radius: 12px;
    padding: 16px 20px;
    box-shadow: 0 1px 4px rgba(0,0,0,0.06);
    transition: box-shadow 0.2s ease;
}}
.kpi-card:hover {{
    box-shadow: 0 3px 12px rgba(0,0,0,0.10);
}}
.kpi-label {{
    font-size: 0.78rem;
    color: #6c757d;
    text-transform: uppercase;
    letter-spacing: 0.4px;
    font-weight: 600;
    margin-bottom: 4px;
}}
.kpi-value {{
    font-size: 1.45rem;
    font-weight: 700;
    color: #212529;
    line-height: 1.2;
}}
.kpi-value.positive {{ color: #198754; }}
.kpi-value.negative {{ color: #dc3545; }}
.kpi-value.info {{ color: {OLIVE_GREEN}; }}
.kpi-value.warning {{ color: #e67e22; }}
@media (max-width: 640px) {{
    .kpi-card {{ flex: 1 1 100%; }}
    .kpi-value {{ font-size: 1.25rem; }}
}}
</style>
""", unsafe_allow_html=True)


# ============================================================
# YARDIMCI FONKSİYONLAR
# ============================================================

def render_kpi_cards(cards: list):
    """Profesyonel KPI kartları render eder.
    cards: [{"icon": "🛢️", "label": "...", "value": "...", "color": ""}, ...]
    color: "", "positive", "negative", "info", "warning"
    """
    html = '<div class="kpi-grid">'
    for c in cards:
        color_cls = c.get("color", "")
        icon = c.get("icon", "")
        html += f'''
        <div class="kpi-card">
            <div class="kpi-label">{icon} {c["label"]}</div>
            <div class="kpi-value {color_cls}">{c["value"]}</div>
        </div>'''
    html += '</div>'
    st.markdown(html, unsafe_allow_html=True)


def hash_password(password: str) -> str:
    return hashlib.sha256(password.encode()).hexdigest()


def get_db() -> sqlite3.Connection:
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA journal_mode=WAL")
    conn.execute("PRAGMA foreign_keys=ON")
    return conn


def init_db():
    conn = get_db()
    cur = conn.cursor()
    cur.executescript("""
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            username TEXT UNIQUE NOT NULL,
            password_hash TEXT NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
        CREATE TABLE IF NOT EXISTS customers (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL,
            phone TEXT,
            address TEXT,
            durum INTEGER DEFAULT 1,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
        CREATE TABLE IF NOT EXISTS oil_purchases (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            customer_id INTEGER NOT NULL,
            purchase_date DATE NOT NULL,
            kg REAL NOT NULL,
            acidity REAL,
            unit_price REAL NOT NULL,
            total_amount REAL NOT NULL,
            note TEXT,
            photo_path TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            created_by TEXT,
            FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE CASCADE
        );
        CREATE TABLE IF NOT EXISTS payments (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            customer_id INTEGER NOT NULL,
            payment_date DATE NOT NULL,
            amount REAL NOT NULL,
            payment_type TEXT NOT NULL,
            note TEXT,
            receipt_photo TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            created_by TEXT,
            FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE CASCADE
        );
        CREATE TABLE IF NOT EXISTS sales (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            sale_date DATE NOT NULL,
            buyer_company TEXT NOT NULL,
            acidity REAL NOT NULL,
            kg REAL NOT NULL,
            unit_price REAL NOT NULL,
            total_amount REAL NOT NULL,
            note TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            created_by TEXT
        );
    """)

    try:
        cur.execute("ALTER TABLE customers ADD COLUMN durum INTEGER DEFAULT 1")
    except sqlite3.OperationalError:
        pass

    existing = cur.execute("SELECT id FROM users WHERE username = ?", (DEFAULT_USER,)).fetchone()
    if not existing:
        cur.execute("INSERT INTO users (username, password_hash) VALUES (?, ?)",
                    (DEFAULT_USER, hash_password(DEFAULT_PASS)))
    conn.commit()
    conn.close()


def save_uploaded_file(uploaded_file, subfolder: str = "") -> str:
    if uploaded_file is None:
        return ""
    target_dir = os.path.join(UPLOAD_DIR, subfolder)
    os.makedirs(target_dir, exist_ok=True)
    ext = Path(uploaded_file.name).suffix
    filename = f"{uuid.uuid4().hex}{ext}"
    filepath = os.path.join(target_dir, filename)
    with open(filepath, "wb") as f:
        f.write(uploaded_file.getbuffer())
    return filepath


def format_currency(amount: float) -> str:
    return f"₺{amount:,.2f}"


def format_kg(kg: float) -> str:
    return f"{kg:,.1f} kg"


# ============================================================
# VERİTABANI SORGULARI - KULLANICILAR
# ============================================================

def get_users():
    conn = get_db()
    rows = conn.execute("SELECT id, username, created_at FROM users ORDER BY username").fetchall()
    conn.close()
    return rows

def add_user(username, password):
    conn = get_db()
    try:
        conn.execute("INSERT INTO users (username, password_hash) VALUES (?, ?)",
                     (username, hash_password(password)))
        conn.commit()
        return True
    except sqlite3.IntegrityError:
        return False
    finally:
        conn.close()

def change_password(user_id, new_password):
    conn = get_db()
    conn.execute("UPDATE users SET password_hash = ? WHERE id = ?",
                 (hash_password(new_password), user_id))
    conn.commit()
    conn.close()

def change_username(user_id, new_username):
    conn = get_db()
    try:
        conn.execute("UPDATE users SET username = ? WHERE id = ?",
                     (new_username, user_id))
        conn.commit()
        return True
    except sqlite3.IntegrityError:
        return False
    finally:
        conn.close()

def delete_user(user_id):
    conn = get_db()
    conn.execute("DELETE FROM users WHERE id = ?", (user_id,))
    conn.commit()
    conn.close()

# ============================================================
# VERİTABANI SORGULARI
# ============================================================

def add_customer(name, phone, address):
    conn = get_db()
    conn.execute("INSERT INTO customers (name, phone, address) VALUES (?, ?, ?)",
                 (name, phone, address))
    conn.commit()
    conn.close()


def get_customers(only_active=False):
    conn = get_db()
    if only_active:
        rows = conn.execute("SELECT * FROM customers WHERE durum = 1 ORDER BY name").fetchall()
    else:
        rows = conn.execute("SELECT * FROM customers ORDER BY name").fetchall()
    conn.close()
    return rows


def delete_customer(customer_id):
    conn = get_db()
    conn.execute("DELETE FROM customers WHERE id = ?", (customer_id,))
    conn.commit()
    conn.close()


def toggle_customer_status(customer_id, current_status):
    new_status = 0 if current_status == 1 else 1
    conn = get_db()
    conn.execute("UPDATE customers SET durum = ? WHERE id = ?", (new_status, customer_id))
    conn.commit()
    conn.close()


def add_oil_purchase(customer_id, purchase_date, kg, acidity, unit_price, total_amount, note, photo_path, created_by):
    conn = get_db()
    conn.execute(
        """INSERT INTO oil_purchases
           (customer_id, purchase_date, kg, acidity, unit_price, total_amount, note, photo_path, created_by)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (customer_id, purchase_date, kg, acidity, unit_price, total_amount, note, photo_path, created_by))
    conn.commit()
    conn.close()


def get_oil_purchases(customer_id=None):
    conn = get_db()
    if customer_id:
        rows = conn.execute(
            """SELECT op.*, c.name as customer_name
               FROM oil_purchases op JOIN customers c ON op.customer_id = c.id
               WHERE op.customer_id = ? ORDER BY op.purchase_date DESC""",
            (customer_id,)).fetchall()
    else:
        rows = conn.execute(
            """SELECT op.*, c.name as customer_name
               FROM oil_purchases op JOIN customers c ON op.customer_id = c.id
               ORDER BY op.purchase_date DESC""").fetchall()
    conn.close()
    return rows


def delete_oil_purchase(purchase_id):
    conn = get_db()
    conn.execute("DELETE FROM oil_purchases WHERE id = ?", (purchase_id,))
    conn.commit()
    conn.close()


def add_payment(customer_id, payment_date, amount, payment_type, note, receipt_photo, created_by):
    conn = get_db()
    conn.execute(
        """INSERT INTO payments
           (customer_id, payment_date, amount, payment_type, note, receipt_photo, created_by)
           VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (customer_id, payment_date, amount, payment_type, note, receipt_photo, created_by))
    conn.commit()
    conn.close()


def get_payments(customer_id=None):
    conn = get_db()
    if customer_id:
        rows = conn.execute(
            """SELECT p.*, c.name as customer_name
               FROM payments p JOIN customers c ON p.customer_id = c.id
               WHERE p.customer_id = ? ORDER BY p.payment_date DESC""",
            (customer_id,)).fetchall()
    else:
        rows = conn.execute(
            """SELECT p.*, c.name as customer_name
               FROM payments p JOIN customers c ON p.customer_id = c.id
               ORDER BY p.payment_date DESC""").fetchall()
    conn.close()
    return rows


def delete_payment(payment_id):
    conn = get_db()
    conn.execute("DELETE FROM payments WHERE id = ?", (payment_id,))
    conn.commit()
    conn.close()


def add_sale(sale_date, buyer_company, acidity, kg, unit_price, total_amount, note, created_by):
    conn = get_db()
    conn.execute(
        """INSERT INTO sales
           (sale_date, buyer_company, acidity, kg, unit_price, total_amount, note, created_by)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        (sale_date, buyer_company, acidity, kg, unit_price, total_amount, note, created_by))
    conn.commit()
    conn.close()


def get_sales():
    conn = get_db()
    rows = conn.execute("SELECT * FROM sales ORDER BY sale_date DESC").fetchall()
    conn.close()
    return rows


def delete_sale(sale_id):
    conn = get_db()
    conn.execute("DELETE FROM sales WHERE id = ?", (sale_id,))
    conn.commit()
    conn.close()


def get_acidity_details(acidity):
    conn = get_db()
    rows = conn.execute("""
        SELECT op.purchase_date, op.kg, c.name as customer_name
        FROM oil_purchases op
        JOIN customers c ON op.customer_id = c.id
        WHERE op.acidity = ?
        ORDER BY op.purchase_date DESC
    """, (acidity,)).fetchall()
    conn.close()
    return rows


def get_stock_by_acidity():
    """Asit derecesine göre net stok hesaplar (Alım - Satış)."""
    conn = get_db()
    rows = conn.execute("""
        SELECT
            acidity,
            COALESCE(purchased, 0) as purchased,
            COALESCE(sold, 0) as sold,
            COALESCE(purchased, 0) - COALESCE(sold, 0) as net_stock
        FROM (
            SELECT acidity, SUM(kg) as purchased
            FROM oil_purchases
            WHERE acidity IS NOT NULL AND acidity > 0
            GROUP BY acidity
        ) purchases
        LEFT JOIN (
            SELECT acidity as s_acidity, SUM(kg) as sold
            FROM sales
            GROUP BY acidity
        ) sales_tbl ON purchases.acidity = sales_tbl.s_acidity

        UNION

        SELECT
            s_acidity as acidity,
            COALESCE(purchased, 0) as purchased,
            COALESCE(sold, 0) as sold,
            COALESCE(purchased, 0) - COALESCE(sold, 0) as net_stock
        FROM (
            SELECT acidity as s_acidity, SUM(kg) as sold
            FROM sales
            GROUP BY acidity
        ) sales_only
        LEFT JOIN (
            SELECT acidity, SUM(kg) as purchased
            FROM oil_purchases
            WHERE acidity IS NOT NULL AND acidity > 0
            GROUP BY acidity
        ) purchases_tbl ON sales_only.s_acidity = purchases_tbl.acidity
        WHERE purchases_tbl.acidity IS NULL

        ORDER BY acidity
    """).fetchall()
    conn.close()
    return rows


def get_available_stock(acidity_value: float) -> float:
    """Belirli bir asit derecesindeki mevcut stoku döndürür."""
    conn = get_db()
    purchased = conn.execute(
        "SELECT COALESCE(SUM(kg), 0) as total FROM oil_purchases WHERE acidity = ?",
        (acidity_value,)).fetchone()["total"]
    sold = conn.execute(
        "SELECT COALESCE(SUM(kg), 0) as total FROM sales WHERE acidity = ?",
        (acidity_value,)).fetchone()["total"]
    conn.close()
    return purchased - sold


def get_customer_summary():
    conn = get_db()
    rows = conn.execute("""
        SELECT
            c.id, c.name, c.phone, c.address, c.durum,
            COALESCE(SUM(op.kg), 0) as total_kg,
            COALESCE(SUM(op.total_amount), 0) as total_purchase_amount,
            COALESCE(
                (SELECT SUM(p.amount) FROM payments p WHERE p.customer_id = c.id), 0
            ) as total_paid,
            COALESCE(SUM(op.total_amount), 0) - COALESCE(
                (SELECT SUM(p.amount) FROM payments p WHERE p.customer_id = c.id), 0
            ) as balance
        FROM customers c
        LEFT JOIN oil_purchases op ON c.id = op.customer_id
        GROUP BY c.id
        ORDER BY c.name
    """).fetchall()
    conn.close()
    return rows


def get_dashboard_stats():
    conn = get_db()
    stats = {}
    stats["total_customers"] = conn.execute("SELECT COUNT(*) as cnt FROM customers").fetchone()["cnt"]

    row = conn.execute(
        "SELECT COALESCE(SUM(kg),0) as total_kg, COALESCE(SUM(total_amount),0) as total_amount FROM oil_purchases"
    ).fetchone()
    stats["total_kg"] = row["total_kg"]
    stats["total_purchase_amount"] = row["total_amount"]

    stats["total_paid"] = conn.execute(
        "SELECT COALESCE(SUM(amount),0) as total_paid FROM payments").fetchone()["total_paid"]
    stats["total_balance"] = stats["total_purchase_amount"] - stats["total_paid"]
    stats["total_purchases"] = conn.execute(
        "SELECT COUNT(*) as cnt FROM oil_purchases").fetchone()["cnt"]

    # Depo istatistikleri
    stats["total_sold_kg"] = conn.execute(
        "SELECT COALESCE(SUM(kg),0) as total FROM sales").fetchone()["total"]
    stats["total_sale_amount"] = conn.execute(
        "SELECT COALESCE(SUM(total_amount),0) as total FROM sales").fetchone()["total"]
    stats["net_stock_kg"] = stats["total_kg"] - stats["total_sold_kg"]

    conn.close()
    return stats


# ============================================================
# SAYFA FONKSİYONLARI
# ============================================================

def render_login():
    st.markdown("""
    <div class="brand-header">
        <div class="brand-name">🫒 Olivarda</div>
        <div class="brand-subtitle">Erengül Zeytinyağı Ticareti</div>
        <div class="brand-divider"></div>
    </div>
    """, unsafe_allow_html=True)
    st.write("")
    st.divider()

    username = st.text_input("Kullanıcı Adı", placeholder="admin")
    password = st.text_input("Şifre", type="password", placeholder="••••")

    if st.button("🚀 Giriş Yap", use_container_width=True):
        if not username or not password:
            st.error("Kullanıcı adı ve şifre boş bırakılamaz.")
            return
        conn = get_db()
        user = conn.execute(
            "SELECT * FROM users WHERE username = ? AND password_hash = ?",
            (username, hash_password(password))).fetchone()
        conn.close()
        if user:
            st.session_state["authenticated"] = True
            st.session_state["username"] = username
            st.rerun()
        else:
            st.error("❌ Kullanıcı adı veya şifre hatalı!")

    st.info("Varsayılan giriş: **admin** / **1234**")


def render_dashboard():
    st.header("📊 Genel Bakış")
    st.caption("Tüm işlemlerinizin özet görünümü")

    stats = get_dashboard_stats()

    render_kpi_cards([
        {"icon": "👥", "label": "Toplam Müşteri", "value": stats["total_customers"], "color": "info"},
        {"icon": "🛢️", "label": "Toplam Alınan Yağ", "value": format_kg(stats["total_kg"])},
        {"icon": "📦", "label": "Depoda Kalan Yağ", "value": format_kg(stats["net_stock_kg"]), "color": "info"},
        {"icon": "💰", "label": "Toplam Hak Ediş", "value": format_currency(stats["total_purchase_amount"]), "color": "warning"},
        {"icon": "💸", "label": "Toplam Ödenen", "value": format_currency(stats["total_paid"]), "color": "positive"},
        {"icon": "📋", "label": "Kalan Bakiye", "value": format_currency(stats["total_balance"]),
         "color": "negative" if stats["total_balance"] > 0 else "positive"},
        {"icon": "🏷️", "label": "Toplam Satış Geliri", "value": format_currency(stats["total_sale_amount"]), "color": "positive"},
    ])

    st.divider()

    st.subheader("🕐 Son Yağ Alımları")
    recent_purchases = get_oil_purchases()[:5]
    if recent_purchases:
        for p in recent_purchases:
            with st.container(border=True):
                st.write(f"**{p['customer_name']}** — {format_currency(p['total_amount'])}")
                st.caption(f"{p['purchase_date']} · {format_kg(p['kg'])} · Asit: %{p['acidity'] or '-'}")
    else:
        st.info("Henüz yağ alım kaydı yok.")

    st.subheader("💸 Son Ödemeler")
    recent_payments = get_payments()[:5]
    if recent_payments:
        for p in recent_payments:
            with st.container(border=True):
                st.write(f"**{p['customer_name']}** — {format_currency(p['amount'])}")
                st.caption(f"{p['payment_date']} · {p['payment_type']}")
    else:
        st.info("Henüz ödeme kaydı yok.")


def render_customers():
    st.header("👥 Müşteri Yönetimi")
    st.caption("Müşterilerinizi ekleyin ve cari hesaplarını takip edin")

    tab_list, tab_new = st.tabs(["📋 Müşteri Listesi", "➕ Yeni Müşteri"])

    with tab_list:
        summaries = get_customer_summary()
        if not summaries:
            st.info("Henüz müşteri eklenmemiş. '➕ Yeni Müşteri' sekmesinden ekleyebilirsiniz.")
        else:
            search = st.text_input("🔍 Müşteri Ara", placeholder="İsim veya telefon ile arayın...")

            for row in summaries:
                if search:
                    s = search.lower()
                    if s not in row["name"].lower() and s not in (row["phone"] or "").lower():
                        continue

                balance = row["balance"]
                if balance > 0:
                    badge = f"🔴 Borç: {format_currency(balance)}"
                elif balance < 0:
                    badge = f"🟢 Alacak: {format_currency(abs(balance))}"
                else:
                    badge = "✅ Hesap Kapalı"

                with st.container(border=True):
                    status_val = row["durum"] if row["durum"] is not None else 1
                    status_text = "🟢 Aktif" if status_val == 1 else "🔴 Pasif"
                    st.write(f"**{row['name']}**  ({status_text})")
                    st.caption(f"📞 {row['phone'] or '—'}  ·  📍 {row['address'] or '—'}")
                    st.write(badge)
                    st.caption(
                        f"🛢️ Yağ: {format_kg(row['total_kg'])}  ·  "
                        f"💰 Hak Ediş: {format_currency(row['total_purchase_amount'])}  ·  "
                        f"💸 Ödenen: {format_currency(row['total_paid'])}  ·  "
                        f"📋 Bakiye: {format_currency(balance)}"
                    )
                    
                    if st.button("🔄 Durumu Değiştir", key=f"toggle_cust_{row['id']}"):
                        toggle_customer_status(row["id"], status_val)
                        st.success(f"✅ {row['name']} durumu güncellendi.")
                        st.rerun()
                        
                    if st.button("🗑️ Sil", key=f"del_cust_{row['id']}"):
                        delete_customer(row["id"])
                        st.success(f"✅ {row['name']} silindi.")
                        st.rerun()

    with tab_new:
        st.subheader("Yeni Müşteri Ekle")
        with st.form("new_customer_form", clear_on_submit=True):
            name = st.text_input("Ad Soyad *", placeholder="Örn: Ahmet Yılmaz")
            phone = st.text_input("Telefon", placeholder="Örn: 0532 123 4567")
            address = st.text_input("Adres / Bölge", placeholder="Örn: Aydın / Germencik")
            submitted = st.form_submit_button("✅ Müşteriyi Kaydet", use_container_width=True)

            if submitted:
                if not name.strip():
                    st.error("Ad Soyad alanı zorunludur!")
                else:
                    add_customer(name.strip(), phone.strip(), address.strip())
                    st.success(f"✅ **{name}** başarıyla eklendi!")


def render_oil_purchase():
    st.header("🛢️ Yağ Alım Kaydı")
    st.caption("Müstahsilden gelen zeytinyağı alımlarını kaydedin")

    tab_new, tab_history = st.tabs(["➕ Yeni Alım", "📋 Alım Geçmişi"])

    with tab_new:
        customers = get_customers(only_active=True)
        if not customers:
            st.warning("⚠️ Aktif müşteri bulunamadı! Lütfen önce aktif bir müşteri ekleyin.")
        else:
            customer_options = {f"{c['name']} ({c['phone'] or 'Tel yok'})": c["id"] for c in customers}

            with st.form("new_oil_form", clear_on_submit=True):
                selected = st.selectbox("Müşteri Seçin *", list(customer_options.keys()), index=None, placeholder="Lütfen Seçiniz")
                purchase_date = st.date_input("Tarih *", value=date.today())
                kg = st.number_input("Miktar (KG) *", min_value=0.0, step=0.5, format="%.1f")
                acidity = st.number_input("Asit Derecesi (Dizyem)", min_value=0.0, max_value=100.0, step=0.1, format="%.1f")
                unit_price = st.number_input("KG Birim Fiyatı (₺) *", min_value=0.0, step=1.0, format="%.2f")

                # Otomatik toplam önizleme (form içinde hesaplanamaz, submit sonrası hesaplanır)
                st.info("💡 Toplam tutar kayıt sırasında otomatik hesaplanır: KG × Birim Fiyat")

                note = st.text_area("Not (Opsiyonel)", placeholder="Varsa açıklama yazın...")
                photo = st.file_uploader("Fotoğraf (Opsiyonel)", type=["jpg", "jpeg", "png", "webp"])
                submitted = st.form_submit_button("💾 Alımı Kaydet", use_container_width=True)

                if submitted:
                    if not selected:
                        st.error("Lütfen bir müşteri seçin!")
                    elif kg <= 0:
                        st.error("Miktar 0'dan büyük olmalıdır!")
                    elif unit_price <= 0:
                        st.error("Birim fiyat 0'dan büyük olmalıdır!")
                    else:
                        customer_id = customer_options[selected]
                        total_amount = kg * unit_price
                        photo_path = save_uploaded_file(photo, "oil_photos") if photo else ""
                        active_user = st.session_state.get("username", "admin")
                        add_oil_purchase(customer_id, purchase_date.isoformat(), kg, acidity,
                                         unit_price, total_amount, note, photo_path, active_user)
                        st.success(f"✅ {format_kg(kg)} yağ alımı kaydedildi! Toplam: {format_currency(total_amount)}")

    with tab_history:
        customers = get_customers()
        filter_options = {"Tüm Müşteriler": None}
        filter_options.update({c["name"]: c["id"] for c in customers})
        selected_filter = st.selectbox("Müşteriye Göre Filtrele", list(filter_options.keys()), key="oil_filter")
        filter_id = filter_options[selected_filter]

        purchases = get_oil_purchases(filter_id)
        if not purchases:
            st.info("Kayıtlı yağ alımı bulunamadı.")
        else:
            for p in purchases:
                with st.container(border=True):
                    st.write(f"**{p['customer_name']}** — {format_currency(p['total_amount'])}")
                    st.caption(
                        f"{p['purchase_date']}  ·  ⚖️ {format_kg(p['kg'])}  ·  "
                        f"💲 {format_currency(p['unit_price'])}/kg  ·  🧪 %{p['acidity'] or '-'}"
                    )
                    st.caption(f"👤 İşlem Yapan: {p['created_by'] if p['created_by'] else 'Bilinmiyor'}")
                    if p["note"]:
                        st.caption(f"📝 {p['note']}")
                    if p["photo_path"] and os.path.exists(p["photo_path"]):
                        with st.expander("📸 Fotoğrafı Gör"):
                            st.image(p["photo_path"], width=300)
                    if st.button("🗑️ Sil", key=f"del_oil_{p['id']}"):
                        delete_oil_purchase(p["id"])
                        st.success("Kayıt silindi.")
                        st.rerun()


def render_payments():
    st.header("💸 Ödeme İşlemleri")
    st.caption("Müşterilere yapılan ödemeleri kaydedin ve takip edin")

    tab_new, tab_history = st.tabs(["➕ Yeni Ödeme", "📋 Ödeme Geçmişi"])

    with tab_new:
        customers = get_customers(only_active=True)
        if not customers:
            st.warning("⚠️ Aktif müşteri bulunamadı! Lütfen önce aktif bir müşteri ekleyin.")
        else:
            customer_options = {f"{c['name']} ({c['phone'] or 'Tel yok'})": c["id"] for c in customers}

            # Müşteri seçimi form dışında (bakiye göstermek için)
            selected = st.selectbox("Müşteri Seçin *", list(customer_options.keys()), index=None, placeholder="Lütfen Seçiniz", key="pay_customer")
            
            if selected:
                customer_id = customer_options[selected]
                # Bakiye bilgisi
                summaries = get_customer_summary()
                s = next((x for x in summaries if x["id"] == customer_id), None)
                if s:
                    render_kpi_cards([
                        {"icon": "💰", "label": "Hak Ediş", "value": format_currency(s["total_purchase_amount"]), "color": "warning"},
                        {"icon": "💸", "label": "Ödenen", "value": format_currency(s["total_paid"]), "color": "positive"},
                        {"icon": "📋", "label": "Güncel Bakiye", "value": format_currency(s["balance"]),
                         "color": "negative" if s["balance"] > 0 else "positive"},
                    ])
            else:
                customer_id = None

            with st.form("new_payment_form", clear_on_submit=True):
                payment_date = st.date_input("Ödeme Tarihi *", value=date.today())
                amount = st.number_input("Ödenen Tutar (₺) *", min_value=0.0, step=100.0, format="%.2f")
                payment_type = st.radio("Ödeme Türü *", ["Nakit", "Havale"], horizontal=True)
                note = st.text_area("Not (Opsiyonel)", placeholder="Varsa açıklama yazın...")
                receipt = st.file_uploader("Makbuz Fotoğrafı (Opsiyonel)", type=["jpg", "jpeg", "png", "webp"])
                submitted = st.form_submit_button("💾 Ödemeyi Kaydet", use_container_width=True)

                if submitted:
                    if not customer_id:
                        st.error("Lütfen bir müşteri seçin!")
                    elif amount <= 0:
                        st.error("Ödeme tutarı 0'dan büyük olmalıdır!")
                    else:
                        receipt_path = save_uploaded_file(receipt, "receipts") if receipt else ""
                        active_user = st.session_state.get("username", "admin")
                        add_payment(customer_id, payment_date.isoformat(), amount, payment_type, note, receipt_path, active_user)
                        st.success(f"✅ {format_currency(amount)} ödeme kaydedildi!")

    with tab_history:
        customers = get_customers()
        filter_options = {"Tüm Müşteriler": None}
        filter_options.update({c["name"]: c["id"] for c in customers})
        selected_filter = st.selectbox("Müşteriye Göre Filtrele", list(filter_options.keys()), key="pay_filter")
        filter_id = filter_options[selected_filter]

        payments = get_payments(filter_id)
        if not payments:
            st.info("Kayıtlı ödeme bulunamadı.")
        else:
            for p in payments:
                with st.container(border=True):
                    st.write(f"**{p['customer_name']}** — {format_currency(p['amount'])}")
                    st.caption(f"{p['payment_date']}  ·  💳 {p['payment_type']}")
                    st.caption(f"👤 İşlem Yapan: {p['created_by'] if p['created_by'] else 'Bilinmiyor'}")
                    if p["note"]:
                        st.caption(f"📝 {p['note']}")
                    if p["receipt_photo"] and os.path.exists(p["receipt_photo"]):
                        with st.expander("🧾 Makbuzu Gör"):
                            st.image(p["receipt_photo"], width=300)
                    if st.button("🗑️ Sil", key=f"del_pay_{p['id']}"):
                        delete_payment(p["id"])
                        st.success("Ödeme silindi.")
                        st.rerun()


def render_warehouse():
    st.header("📦 Depo ve Satış")
    st.caption("Stok durumunu takip edin ve satış kayıtlarını girin")

    tab_stock, tab_new_sale, tab_history = st.tabs(["📊 Stok Durumu", "➕ Yeni Satış", "📋 Satış Geçmişi"])

    # --- STOK DURUMU ---
    with tab_stock:
        st.subheader("Depo Stok Durumu (Asit Bazında)")

        stock_rows = get_stock_by_acidity()
        if not stock_rows:
            st.info("Henüz depoda kayıtlı yağ bulunmuyor.")
        else:
            # Genel toplam
            total_purchased = sum(r["purchased"] for r in stock_rows)
            total_sold = sum(r["sold"] for r in stock_rows)
            total_net = sum(r["net_stock"] for r in stock_rows)

            render_kpi_cards([
                {"icon": "🛢️", "label": "Toplam Alınan", "value": format_kg(total_purchased)},
                {"icon": "🏷️", "label": "Toplam Satılan", "value": format_kg(total_sold), "color": "warning"},
                {"icon": "📦", "label": "Depoda Kalan", "value": format_kg(total_net), "color": "info"},
            ])

            st.divider()

            # Asit bazında detay
            for r in stock_rows:
                net = r["net_stock"]
                if net > 0:
                    icon = "🟢"
                elif net == 0:
                    icon = "⚪"
                else:
                    icon = "🔴"

                with st.container(border=True):
                    st.write(f"{icon} **Asit: %{r['acidity']}**")
                    st.caption(
                        f"Alınan: {format_kg(r['purchased'])}  ·  "
                        f"Satılan: {format_kg(r['sold'])}  ·  "
                        f"**Kalan: {format_kg(net)}**"
                    )
                    with st.expander("🔍 Kaynak Detaylarını Gör"):
                        details = get_acidity_details(r['acidity'])
                        if details:
                            for d in details:
                                st.write(f"- **{d['customer_name']}**: {format_kg(d['kg'])} ({d['purchase_date']})")
                        else:
                            st.caption("Detay bulunamadı.")

    # --- YENİ SATIŞ ---
    with tab_new_sale:
        st.subheader("Yeni Satış Kaydı")

        stock_rows = get_stock_by_acidity()
        available_acidities = [r for r in stock_rows if r["net_stock"] > 0]

        if not available_acidities:
            st.warning("⚠️ Depoda satılacak yağ bulunmuyor. Önce yağ alımı yapmanız gerekiyor.")
        else:
            # Mevcut stok bilgisi
            st.info("📦 **Mevcut Stok:**  " + "  ·  ".join(
                [f"%{r['acidity']} → {format_kg(r['net_stock'])}" for r in available_acidities]
            ))

            acidity_options = [r["acidity"] for r in available_acidities]

            with st.form("new_sale_form", clear_on_submit=True):
                sale_date = st.date_input("Satış Tarihi *", value=date.today())
                buyer = st.text_input("Alıcı Firma *", placeholder="Örn: Tariş Zeytin A.Ş.")
                selected_acidity = st.selectbox(
                    "Asit Derecesi *",
                    options=acidity_options,
                    index=None,
                    placeholder="Lütfen Seçiniz",
                    format_func=lambda x: f"%{x}"
                )
                sale_kg = st.number_input("Miktar (KG) *", min_value=0.0, step=0.5, format="%.1f")
                sale_price = st.number_input("KG Birim Fiyatı (₺) *", min_value=0.0, step=1.0, format="%.2f")

                st.info("💡 Toplam tutar kayıt sırasında otomatik hesaplanır: KG × Birim Fiyat")

                sale_note = st.text_area("Not (Opsiyonel)", placeholder="Varsa açıklama yazın...")
                submitted = st.form_submit_button("💾 Satışı Kaydet", use_container_width=True)

                if submitted:
                    if not selected_acidity:
                        st.error("Lütfen asit derecesi seçin!")
                    elif not buyer.strip():
                        st.error("Alıcı firma adı zorunludur!")
                    elif sale_kg <= 0:
                        st.error("Miktar 0'dan büyük olmalıdır!")
                    elif sale_price <= 0:
                        st.error("Birim fiyat 0'dan büyük olmalıdır!")
                    else:
                        # Stok kontrolü
                        available = get_available_stock(selected_acidity)
                        if sale_kg > available:
                            st.error(
                                f"🚫 **STOK YETERSİZ!** "
                                f"%{selected_acidity} asit derecesinde depoda yalnızca "
                                f"**{format_kg(available)}** yağ var. "
                                f"{format_kg(sale_kg)} satış yapılamaz!"
                            )
                        else:
                            total_sale = sale_kg * sale_price
                            active_user = st.session_state.get("username", "admin")
                            add_sale(sale_date.isoformat(), buyer.strip(), selected_acidity,
                                     sale_kg, sale_price, total_sale, sale_note, active_user)
                            st.success(
                                f"✅ Satış kaydedildi! "
                                f"{format_kg(sale_kg)} (%{selected_acidity} asit) → "
                                f"**{buyer}** · Toplam: {format_currency(total_sale)}"
                            )

    # --- SATIŞ GEÇMİŞİ ---
    with tab_history:
        st.subheader("Satış Geçmişi")
        sales = get_sales()
        if not sales:
            st.info("Henüz satış kaydı bulunmuyor.")
        else:
            for s in sales:
                with st.container(border=True):
                    st.write(f"**{s['buyer_company']}** — {format_currency(s['total_amount'])}")
                    st.caption(
                        f"{s['sale_date']}  ·  ⚖️ {format_kg(s['kg'])}  ·  "
                        f"🧪 %{s['acidity']}  ·  💲 {format_currency(s['unit_price'])}/kg"
                    )
                    st.caption(f"👤 İşlem Yapan: {s['created_by'] if s['created_by'] else 'Bilinmiyor'}")
                    if s["note"]:
                        st.caption(f"📝 {s['note']}")
                    if st.button("🗑️ Sil", key=f"del_sale_{s['id']}"):
                        delete_sale(s["id"])
                        st.success("Satış kaydı silindi.")
                        st.rerun()


def render_customer_detail():
    st.header("🔎 Müşteri Detayı")
    st.caption("Bir müşterinin tüm alım ve ödeme geçmişini görüntüleyin")

    customers = get_customers()
    if not customers:
        st.info("Henüz müşteri eklenmemiş.")
        return

    customer_options = {c["name"]: c["id"] for c in customers}
    selected = st.selectbox("Müşteri Seçin", list(customer_options.keys()), key="detail_customer")
    customer_id = customer_options[selected]

    summaries = get_customer_summary()
    summary = next((x for x in summaries if x["id"] == customer_id), None)

    if summary:
        render_kpi_cards([
            {"icon": "🛢️", "label": "Toplam Yağ", "value": format_kg(summary["total_kg"])},
            {"icon": "💰", "label": "Hak Ediş", "value": format_currency(summary["total_purchase_amount"]), "color": "warning"},
            {"icon": "💸", "label": "Ödenen", "value": format_currency(summary["total_paid"]), "color": "positive"},
            {"icon": "📋", "label": "Bakiye", "value": format_currency(summary["balance"]),
             "color": "negative" if summary["balance"] > 0 else "positive"},
        ])

    st.divider()

    det_tab1, det_tab2 = st.tabs(["🛢️ Yağ Alımları", "💸 Ödemeler"])

    with det_tab1:
        purchases = get_oil_purchases(customer_id)
        if not purchases:
            st.info("Bu müşteriye ait alım kaydı yok.")
        else:
            for p in purchases:
                with st.container(border=True):
                    st.write(f"**{format_currency(p['total_amount'])}**")
                    st.caption(
                        f"{p['purchase_date']}  ·  ⚖️ {format_kg(p['kg'])}  ·  "
                        f"💲 {format_currency(p['unit_price'])}/kg  ·  🧪 %{p['acidity'] or '-'}"
                    )
                    if p["note"]:
                        st.caption(f"📝 {p['note']}")

    with det_tab2:
        payments = get_payments(customer_id)
        if not payments:
            st.info("Bu müşteriye ait ödeme kaydı yok.")
        else:
            for p in payments:
                with st.container(border=True):
                    st.write(f"**{format_currency(p['amount'])}**")
                    st.caption(f"{p['payment_date']}  ·  {p['payment_type']}")
                    if p["note"]:
                        st.caption(f"📝 {p['note']}")


def render_settings_users():
    st.header("⚙️ Ayarlar / Kullanıcılar")
    st.caption("Kullanıcıları yönetin ve şifrenizi güncelleyin")

    tab_users, tab_profile = st.tabs(["👥 Sistem Kullanıcıları", "🔑 Profilimi Güncelle"])

    with tab_users:
        st.subheader("Mevcut Kullanıcılar")
        users = get_users()
        for u in users:
            with st.container(border=True):
                # Using standard layout instead of columns for mobile compatibility as per user instructions
                st.write(f"**👤 {u['username']}**")
                st.caption(f"Kayıt Tarihi: {u['created_at']}")
                if u['username'] != 'admin' and u['username'] != st.session_state.username:
                    if st.button("🗑️ Sil", key=f"del_usr_{u['id']}"):
                        delete_user(u['id'])
                        st.success("Kullanıcı silindi.")
                        st.rerun()

        st.divider()
        st.subheader("Yeni Kullanıcı Ekle")
        with st.form("new_user_form", clear_on_submit=True):
            new_username = st.text_input("Kullanıcı Adı *")
            new_password = st.text_input("Şifre *", type="password")
            submitted = st.form_submit_button("✅ Ekle", use_container_width=True)
            if submitted:
                if not new_username.strip() or not new_password:
                    st.error("Kullanıcı adı ve şifre zorunludur.")
                else:
                    if add_user(new_username.strip(), new_password):
                        st.success(f"✅ {new_username} eklendi.")
                        st.rerun()
                    else:
                        st.error("Bu kullanıcı adı zaten mevcut.")

    with tab_profile:
        st.subheader("Şifremi Değiştir")
        with st.form("change_password_form", clear_on_submit=True):
            current_user = next((u for u in get_users() if u['username'] == st.session_state.username), None)
            new_pass = st.text_input("Yeni Şifre *", type="password")
            new_pass_confirm = st.text_input("Yeni Şifre (Tekrar) *", type="password")
            submitted = st.form_submit_button("💾 Şifreyi Güncelle", use_container_width=True)
            if submitted:
                if new_pass != new_pass_confirm:
                    st.error("Şifreler uyuşmuyor!")
                elif len(new_pass) < 4:
                    st.error("Şifre en az 4 karakter olmalıdır.")
                elif current_user:
                    change_password(current_user['id'], new_pass)
                    st.success("✅ Şifreniz güncellendi. Lütfen tekrar giriş yapın.")
                    st.session_state["authenticated"] = False
                    st.session_state.pop("username", None)
                    st.rerun()

# ============================================================
# ANA UYGULAMA
# ============================================================

def main():
    init_db()
    os.makedirs(UPLOAD_DIR, exist_ok=True)

    if "authenticated" not in st.session_state:
        st.session_state["authenticated"] = False

    if not st.session_state["authenticated"]:
        render_login()
        return

    with st.sidebar:
        st.markdown("""
        <div class="brand-header">
            <div class="brand-name">🫒 Olivarda</div>
            <div class="brand-subtitle">Erengül Zeytinyağı Ticareti</div>
            <div class="brand-divider"></div>
        </div>
        """, unsafe_allow_html=True)
        st.write("")
        st.divider()

        menu = st.radio(
            "Menü",
            [
                "📊 Genel Bakış",
                "👥 Müşteriler",
                "🛢️ Yağ Alımı",
                "💸 Ödemeler",
                "📦 Depo ve Satış",
                "🔎 Müşteri Detayı",
                "⚙️ Ayarlar / Kullanıcılar",
            ],
            label_visibility="collapsed",
        )

        st.divider()
        st.caption(f"👤 {st.session_state.get('username', 'admin')}")
        st.caption(f"🕐 {datetime.now().strftime('%d.%m.%Y %H:%M')}")

        if st.button("🚪 Çıkış Yap", use_container_width=True):
            st.session_state["authenticated"] = False
            st.session_state.pop("username", None)
            st.rerun()

        st.markdown("""
        <div class="sidebar-footer">
            © 2026 Olivarda — Erengül Zeytinyağı Ticareti
        </div>
        """, unsafe_allow_html=True)

    if menu == "📊 Genel Bakış":
        render_dashboard()
    elif menu == "👥 Müşteriler":
        render_customers()
    elif menu == "🛢️ Yağ Alımı":
        render_oil_purchase()
    elif menu == "💸 Ödemeler":
        render_payments()
    elif menu == "📦 Depo ve Satış":
        render_warehouse()
    elif menu == "🔎 Müşteri Detayı":
        render_customer_detail()
    elif menu == "⚙️ Ayarlar / Kullanıcılar":
        render_settings_users()


if __name__ == "__main__":
    main()
